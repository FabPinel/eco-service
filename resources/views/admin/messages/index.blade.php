...
<div class="flex h-full mt-24 bg-white p-4 flex-col border-x border-stroke dark:border-strokedark">
    ...
    <div class="flex flex-col-reverse justify-between gap-6 py-4.5 pl-4 pr-4 sm:flex-row lg:pl-10 lg:pr-7.5">
      <div class="flex items-center gap-4 p-4">
        ...
        <button>
            <svg class="w-6 h-6 text-red-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="white" viewBox="0 0 24 24">
    ...
    <div x-data="{ showModal: false, showModalResponse : false, id: '', sender: '', enterprise: '', subject: '', email: '', message: '', date: '', hour: '', product: '', phone: '' }" class="h-full">
        ...
        <!-- Modal Content -->
        <div x-show="showModal" @click.away="showModal = false" class="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white p-6 z-50 w-1/2">
            ...
            <div>
                <div class="flex items-center border-b border-slate-200 pb-2">
                    ...
                    @click="showModal = false; showModalResponse = true;"
                    >Répondre</button>
                </div>
        ...
        <div x-show="showModalResponse" class="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white p-6 z-50 w-1/2">
            <form action="{{ route('mailResponse')}}" method="POST">
                @csrf
                <div class="mb-4">
                    <label for="de">De :</label>
                    <input type="email" id="de" name="emailFrom" value="{{ config('mail.from.address') }}" readonly class="border border-blue-900 focus:border-red-500 focus:bg-slate-100 rounded-md py-1 px-2 focus:outline-none">
                </div>
                <div class="mb-4">
                    <label for="a">À :</label>
                ...
                <div class="mb-4">
                    ...
                    Re : <input type="text" id="objet" name="object" x-bind:value="subject">
                </div>
                <div class="mb-4 flex items-start">
                    <label for="message">Message :</label>
                    <textarea id="message" placeholder="Message…" name="message" class="min-w-96 min-h-32"></textarea>
                </div>
                <div class="flex justify-between">
                    <p class="text-lg px-2 cursor-pointer" @click="showModal = true; showModalResponse = false;">Revenir</p>
          ...
          <tr class="flex border-y border-stroke dark:border-strokedark bg-slate-100">
            <th class="w-2/12 py-6 pl-4 pr-4 lg:pl-10">
              <label for="checkbox-1" class="flex cursor-pointer select-none items-center font-medium">
                  <input type="checkbox" class="box mr-4 flex h-5 w-5 items-center bg-white justify-center rounded-[3px] border-[.5px] border-stroke bg-gray-2  dark:border-strokedark dark:bg-boxdark-2">
                Nom Prénom
              </label>
            </th>
